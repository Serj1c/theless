...
func (e *EventService) GetV1(r *http.Request, params *GetV1Params, reply *GetV1Reply) error {
	id := params.Id
	...
	// Getting event item
	err := config.EventsCollection.Find(FindQuery{Slug: eventSlug}).Select(fields).One(&reply.Item)
	...
	query := bson.M{"date": bson.M{"$gte": time.Now()}, "slug": bson.M{"$ne": reply.Item.Slug}}
	fields = bson.M{"address": 1, "cover": 1, "date": 1, "name": 1, "slug": 1, "location": 1}

	// Getting other events
	return config.EventsCollection.Find(query).Select(fields).Limit(3).Sort("date").All(&reply.List)
}
